...
function candidateStatusValue(value) {
...
exports.createCandidateFromCv = async (req, res) => {
  ...
  try {
    ...
    if (jobId) {
      ...
      cvMatch = calculateCvMatch(job, cvText);
    }

    const name = stringValue(req.body.name) || parsed.name;
    const role =
      stringValue(req.body.role) || parsed.role || stringValue(job?.title);
    ...
    const experience = numberValue(req.body.experience, parsed.experience);
    ...
    savedCandidate = await Candidate.create({
      name,
      role,
      status: candidateStatusValue(req.body.status),
      email: stringValue(req.body.email) || parsed.email,
      phone: stringValue(req.body.phone) || parsed.phone,
      education: stringValue(req.body.education) || parsed.education,
      skills: listValue(req.body.skills, parsed.skills),
      experience,
      languages: listValue(req.body.languages, parsed.languages),
      certificates: listValue(req.body.certificates, parsed.certificates),
    ...
    if (job) {
      const matchResult = cvMatch?.match;
    ...
    return res.status(201).json({
      ...
      application,
      match: cvMatch?.match || null,
    });
  } catch (error) {
    console.error("Create candidate from CV error:", error);

    if (savedCandidate?._id) {
      try {
        await Candidate.deleteOne({ _id: savedCandidate._id });
      } catch (cleanupError) {
        console.error("Candidate rollback error:", cleanupError);
      }
    }
    ...
    if (error?.name === "ValidationError") {
      return res.status(400).json({
        message: "CV-dən yaradılan namizəd məlumatları düzgün deyil.",
      });
    }

    return res.status(500).json({
      message: "CV pipeline emal edilərkən server xətası baş verdi.",
    });
  }
};
